...
public:
	...
	void updateRange(const void* origRangePtr, void[] newRange) shared {
		mutex.lock();
		scope(exit) mutex.unlock();
		(cast(MallocRangeBuffer*) &this)
			.updateRangeImpl(origRangePtr, newRange);
	}
	...
	void addRangeImpl(const(void)[] range) {
		...
		if (len == buffer.length) {
			...
			if (buffer.ptr is initial.ptr) {
				...
				memcpy(newPtr, buffer.ptr,
				       buffer.length * (const(void)[]).sizeof);
			} else {
	...
	long findRangeForPtr(const(void*) ptr, bool interior) {
		foreach (i; 0 .. len) {
			if (buffer[i].ptr is ptr)
				return i;
			if (interior && ptr > buffer[i].ptr
				    && ptr - buffer[i].ptr < buffer[i].length)
				return i;
		}
